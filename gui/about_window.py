...
Licencja: MIT License"""
        ...
        github_text = wx.StaticText(panel, label="Zobacz kod źródłowy programu:")
        self.github_link = wx.StaticText(panel, label="https://github.com/pwalczak418/shoper-sitemap-explorer")
        self.github_link.SetForegroundColour(wx.Colour(0, 0, 255))  # Blue link
        font_gh = self.github_link.GetFont()
        font_gh.SetUnderlined(True)
        self.github_link.SetFont(font_gh)
        self.github_link.SetCursor(wx.Cursor(wx.CURSOR_HAND))
        self.github_link.Bind(wx.EVT_LEFT_UP, self.on_github_click)
        ...
        main_sizer.Add(github_text, 0, wx.LEFT | wx.RIGHT, 10)
        main_sizer.Add(self.github_link, 0, wx.LEFT | wx.RIGHT | wx.BOTTOM, 10)
    ...
    def on_github_click(self, event):
        try:
            webbrowser.open("https://github.com/pwalczak418/shoper-sitemap-explorer")
        except Exception as e:
            wx.MessageBox(f"Nie można otworzyć strony: {e}", "Błąd", wx.OK | wx.ICON_ERROR)